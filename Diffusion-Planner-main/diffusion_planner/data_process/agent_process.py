...
def _agent_type_one_hot(agent_type):
    if agent_type == TrackedObjectType.VEHICLE:
        return np.array([1.0, 0.0, 0.0], dtype=np.float32)
    if agent_type == TrackedObjectType.PEDESTRIAN:
        return np.array([0.0, 1.0, 0.0], dtype=np.float32)
    if agent_type == TrackedObjectType.BICYCLE:
        return np.array([0.0, 0.0, 1.0], dtype=np.float32)
    return np.array([0.0, 0.0, 0.0], dtype=np.float32)


def _pad_agent_states_with_mask(agent_trajectories, reverse: bool):
    """
    Pad missing history states with the latest available state and return an
    observation mask. True means the agent was observed at that frame.
    """
    track_id_idx = AgentInternalIndex.track_token()
    ...
    key_frame = agent_trajectories[0]
    num_agents = key_frame.shape[0]
    state_dim = key_frame.shape[1]

    id_row_mapping: Dict[int, int] = {}
    for idx, token in enumerate(key_frame[:, track_id_idx]):
        id_row_mapping[int(token)] = idx

    current_state = np.zeros((num_agents, state_dim), dtype=np.float64)
    padded = np.zeros((len(agent_trajectories), num_agents, state_dim), dtype=np.float64)
    mask = np.zeros((len(agent_trajectories), num_agents), dtype=bool)

    for t, frame in enumerate(agent_trajectories):
        for row_idx in range(frame.shape[0]):
            token = int(frame[row_idx, track_id_idx])
            if token not in id_row_mapping:
                continue

            mapped_row = id_row_mapping[token]
            current_state[mapped_row] = frame[row_idx]
            mask[t, mapped_row] = True

        padded[t] = current_state.copy()
...
def _pad_agent_states_with_zeros_and_mask(agent_trajectories):
    """Pad future states with zeros and return an exact observation mask."""
    key_frame = agent_trajectories[0]
    track_id_idx = AgentInternalIndex.track_token()
    ...
    padded = np.zeros((len(agent_trajectories), key_frame.shape[0], key_frame.shape[1]), dtype=np.float64)
    mask = np.zeros((len(agent_trajectories), key_frame.shape[0]), dtype=bool)
    ...
    for t, frame in enumerate(agent_trajectories):
        for row_idx in range(frame.shape[0]):
            ...
            mapped_row = id_row_mapping[token]
            padded[t, mapped_row] = frame[row_idx]
            mask[t, mapped_row] = True

    return padded, mask


def all_agent_past_future_process(
    anchor_ego_state,
    ...
    """
    Build past and future trajectories for all VEHICLE / PEDESTRIAN / BICYCLE
    agents that exist at the current frame.

    Returns:
        all_neighbor_past: [N, T_h, 11]
        all_neighbor_future: [N, T_f, 3]
        all_neighbor_history_mask: [N, T_h]
        all_neighbor_future_mask: [N, T_f]
        all_neighbor_type: [N, 3]
    """
    agents_states_dim = 8  # x, y, cos h, sin h, vx, vy, width, length

    agent_history = _filter_agents_array(past_tracked_objects, reverse=True)
    current_agent_types = past_tracked_objects_types[-1]
    t_h = len(past_tracked_objects)
    t_f = len(future_tracked_objects) - 1

    if agent_history[-1].shape[0] == 0:
        return (
            np.zeros((0, t_h, agents_states_dim + 3), dtype=np.float32),
            np.zeros((0, t_f, 3), dtype=np.float32),
            np.zeros((0, t_h), dtype=bool),
            np.zeros((0, t_f), dtype=bool),
            np.zeros((0, 3), dtype=np.float32),
        )

    padded_past_states, past_mask = _pad_agent_states_with_mask(agent_history, reverse=True)

    local_past_states = []
    for agent_state in padded_past_states:
        local_past_states.append(
            convert_absolute_quantities_to_relative(agent_state, anchor_ego_state, 'agent')
        )

    num_agents = local_past_states[0].shape[0]
    all_neighbor_past = np.zeros((num_agents, t_h, agents_states_dim + 3), dtype=np.float32)
    all_neighbor_type = np.zeros((num_agents, 3), dtype=np.float32)

    for agent_idx, agent_type in enumerate(current_agent_types):
        if agent_idx < num_agents:
            all_neighbor_type[agent_idx] = _agent_type_one_hot(agent_type)

    for t, state in enumerate(local_past_states):
        all_neighbor_past[:, t, 0] = state[:, AgentInternalIndex.x()]
        all_neighbor_past[:, t, 1] = state[:, AgentInternalIndex.y()]
        all_neighbor_past[:, t, 2] = np.cos(state[:, AgentInternalIndex.heading()])
        all_neighbor_past[:, t, 3] = np.sin(state[:, AgentInternalIndex.heading()])
        all_neighbor_past[:, t, 4] = state[:, AgentInternalIndex.vx()]
        all_neighbor_past[:, t, 5] = state[:, AgentInternalIndex.vy()]
        all_neighbor_past[:, t, 6] = state[:, AgentInternalIndex.width()]
        all_neighbor_past[:, t, 7] = state[:, AgentInternalIndex.length()]
        all_neighbor_past[:, t, 8:11] = all_neighbor_type

    all_neighbor_history_mask = past_mask.T.astype(bool)

    agent_future = _filter_agents_array(future_tracked_objects, reverse=False)
    local_future_states = []
    for agent_state in agent_future:
        local_future_states.append(
            convert_absolute_quantities_to_relative(agent_state, anchor_ego_state, 'agent')
        )

    padded_future_states, future_mask = _pad_agent_states_with_zeros_and_mask(local_future_states)
    future_states = padded_future_states[1:]
    future_mask = future_mask[1:]

    all_neighbor_future = np.zeros((num_agents, future_states.shape[0], 3), dtype=np.float32)
    all_neighbor_future[:, :, 0] = future_states[:, :, AgentInternalIndex.x()].T
    all_neighbor_future[:, :, 1] = future_states[:, :, AgentInternalIndex.y()].T
    all_neighbor_future[:, :, 2] = future_states[:, :, AgentInternalIndex.heading()].T
    all_neighbor_future_mask = future_mask.T.astype(bool)
